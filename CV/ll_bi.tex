\documentclass[10pt,a4paper,english,german]{article}
\usepackage{babel}
\usepackage[latin1]{inputenc}
%\usepackage[top=3cm,left=3cm]{geometry}
%\usepackage{amsmath,amsfonts,amssymb}
\usepackage{graphicx}
\usepackage{multirow}
%\usepackage{fancyhdr}
%%\setlength{\headheight}{15.2pt}
%\pagestyle{fancy}
%\lhead{Andrea Amantini}
%\rhead{Lebeslauf}
%\renewcommand{\headrulewidth}{0.5pt}
%\newcommand{\LN}{\mathcal{LN}}
%\voffset=-100pt
\addtolength{\textheight}{10cm}
\linespread{1.25}
%\pagestyle{empty}
%\pagenumbering{}
\newcommand{\crule}{\rule{13cm}{.4pt}}
%____________________________________________________
%\title{Curriculum Vitae}
\title{Lebenslauf}
\date{}
\begin{document}
\maketitle
%\thispagestyle{empty}
\centering
\begin{tabular}{r@{\extracolsep{2em}}p{5cm}@{ }l}
						&&\multirow{8}{*}{\includegraphics[height=4.75cm]{Lichtbild} }\\ %sonst: cvPhoto1-alta
Name: 					& {\large\bf Andrea Amantini}\\
Geburtsort, Datum: 			& Florenz (Italien), 28.01.1980\\
Staatsangeh\"origkeit: 			& Italienisch\\
Adresse:					& Lausitzerstra{\ss}e 37, 10999 Berlin\\
%Tel:						& +49 (0)30 2093 5826\\[+3mm]
%Privatanschrift (bis 05/08/2008):& Prenzlauer Allee 196, 10405 Berlin\\
Tel:						& +49 (0)30 3462 4345\\
Mob:						& +49 (0)176 2829 7792\\
E-Mail:					& amantini.andrea@gmail.com\\[+15mm]
\end{tabular}
%??????????????????
\begin{tabular}{r@{\extracolsep{2em}}p{9cm}}
\multicolumn{2}{l}{\large\bf Bildungsgang}\\[+1mm]
\hline\\[+1mm]
1994-1999	&Gymnasium ({\em Liceo Scientifico}) -- {\sl Florenz, Italien}\\[+3mm]
1999-2005	&Studium an der {\em Università degli Studi di Firenze} -- {\sl Florenz, Italien}\\
		&Studiengang Mathematik, Abschlussprüfung:
		Diplom ({\sl Laurea in Matematica}) --
		Abschlussnote: 110/110 \emph{cum Laude}\\[+3mm]
%August 2004				&S.M.I Sommer Schule in Perugia, organized by I.N.D.A.M. (Istituto Nazionale di Alta Matematica)\\
Okt 2005 - heute		&Doktorand an der Humboldt Universität zu Berlin\\ 	
				&im Fach Mathematik %schwerpunkt Algebra, Mathematische Logik\\
				({\sl Betreuer Prof. Dr. A. Baudisch})\\[+3mm]
%Okt 2005 - Sep 2008	&Stipendium Marie Curie FP6 -- MODNET\\
Sep 2008 - Okt 2008			&Forschungsaufenthalt an der {\em Université Lyon 1}, Inst. Camille
Jordan -- {\sl Lyon, France}\\[+10mm]
\multicolumn{2}{l}{\large\bf Stellen}\\[+1mm]
\hline\\[+1mm]
Okt 2005 - Sep 2008 	& EU-Gastwissenschaftler an der\\
					&HU Berlin -- Marie Curie (MODNET) Stipendiat\\[+3mm]
Okt 2008 - Dez 2009 	& Wissenschaftlicher Mitarbeiter an der\\
					& HU Berlin\\[+6mm]
\end{tabular}
%??????????????????
\begin{tabular}{r@{\extracolsep{2em}}p{9cm}}
\multicolumn{2}{l}{\large\bf Lehrtätigkeiten an der HUB}\\[+1mm]
\hline\\[+1mm]
WS 08/09	& Übungsleiter im Fach Algebra I\\
SS 09		& Übungsleiter im Fach Gewöhnlichen Differentialgleichungen\\
WS 09/10	& Übungsleiter im Fach Algebra II\\[+6mm]
\multicolumn{2}{l}{\large\bf Sprachen}\\[+1mm]
\hline\\[+1mm]
Italienisch & Muttersprache\\
English & fliessend\\ % gute Kenntnis\\
Deutsch & fliessend {\sl (Zeugnisse  A1.2, B2.2, C1 Deutsch als Fremdsprache -- HU Sprachenzentrum, Berlin)}\\
Franz\"osisch & Grundkenntnisse\\[+5mm]
\multicolumn{2}{l}{\large\bf Betriebssysteme, Programmiersprachen, IDEs}\\[+1mm]
\hline\\[+1mm]
Mac OS X ($\geq 10.4$) 	&gute Kenntnisse\\
Unix/Linux, Windows			&friedlicher Umgang\\
\LaTeX \,``Typesetting" 	&erweiterte Kenntnisse\\
{\sl HTML, CSS}		&Grundkenntnisse\\
Objective-$C$, $C^{++}$ 	&Grundkenntnisse\\
Eclipse, XCode			&Grundkenntnisse
\end{tabular}

\smallskip
%\vfil
%\noindent
\flushright{Berlin, \today}
\end{document}